...
{-# LANGUAGE GADTs #-}
module Ch03.LetrecLang.Types where
...
subsetOfEnv :: [Ident] -> Env -> Env
subsetOfEnv _ EmptyEnv = EmptyEnv
subsetOfEnv vars (ExtendEnv var val env) =
  if var `elem` vars
  then ExtendEnv var val (subsetOfEnv vars env)
  else subsetOfEnv vars env
subsetOfEnv vars (ExtendEnvRec defs env) =
  let procNames = fmap fstOfTriple defs
  in if any (`elem` vars) procNames
     then ExtendEnvRec defs (subsetOfEnv vars env)
     else subsetOfEnv vars env
